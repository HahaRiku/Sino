...
public class ObjActiveController : MonoBehaviour {
    ...
    public struct Element {
        ...
        public string 道具名稱;
        public bool 道具是否在背包;
        public bool 要設定Active的是GameObject還是Component;
        public GameObject 如果是Obj_要設定Active的Obj;
        public GameObject 如果是Comp_要設定Active的Comp;
                ...
                if (e.要使用哪種判斷 == Condition.變數) {
                    if (SystemVariables.IsIntVariableExisted(e.變數名)) {
                        if (SystemVariables.otherVariables_int[e.變數名] == e.如果上面的變數為多少才設置) {
                            e.如果是Obj_要設定Active的Obj.SetActive(e.Active的值);
                            檢查的序列.Remove(e);
                            return;
                        }
                    }
